...
impl UserInputConfigurable for DefaultBehavior {
    fn from_user_input() -> io::Result<Self> {
        ...
        let quiet = get_user_input("[Step 1 of 2] Enable quiet mode, which suppresses output (y/N)");
        let extract_title = get_user_input("[Step 2 of 2] Extract titles from line 1 content (Y/n)");
